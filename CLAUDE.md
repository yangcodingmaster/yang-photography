...
| `archive.html` Archive | ✅ 完成 | 书架式：每年一本书（书脊宽度=照片数）；**翻开 = 跳 `zine.html?year=` 画册**（2026-07-30） |
| `about.html` 关于页 | ✅ 完成 | 单列布局 + 真实 bio + 一排六个社媒图标（2026-08）；og:description 仍是占位 |
| `film.html` Film | ✅ 完成 | 胶卷柜式：每卷一个筒（135/120 两种剪影），抽片头预览；**翻开 = 跳 `zine.html?film=` 画册**（片框跟着搬过去了，2026-07-30）；已录入 7 卷真实胶卷 |
...
| 画册式阅读 | ✅ 全站统一 | Gallery / Archive / Film 三个板块的阅读全部走 `zine.html`（`?id=` / `?year=` / `?film=`，2026-07-30）；三处旧阅读代码保留作回退、无入口。详见"画册式阅读"一节，作者手册见 `画册维护手册.md` |
...
- **书架**：每年是一本书，从左到右按时间排（2021 → 2025），站在搁板细线上
  - 书脊 = 扁平竖条（`site-surface` 底 + `site-border` 边框），竖排年份
  - **书脊宽度 = 16px + 照片数 × 1.6px**（数据即造型：一眼看出哪年拍得多）
  - **分排 = 按宽度装箱**（2026-08 改，取代旧的"手机每排 2 本"）：按真实书脊宽度一排排装，
    约束 = 展开任何一本后仍放得下才继续放；手机（左对齐）允许展开时把右邻推出屏幕边缘，
    桌面（居中）要求整排可见。375px 手机实测 3+2 两排；将来加年份自动溢出新排
  - hover 展宽露出封面（照片图版 + 年份 + 张数）——只在有光标的设备上（`@media (hover: hover)`）
  - **触屏两步交互**：第一次点书脊 = **原地展开**封面预览（左对齐、只向右展宽，前面的书纹丝不动），
    再点封面才翻开；点其他书脊切换预览、点空白收回、Way out 回来自动收回。
    ⚠️ 不做"推到屏幕中心"的整架平移——书展宽和书架平移方向相反会互相抵消，
    视觉上像"从中间被掰开"（Film 页先踩的坑，作者明确否掉，代码注释里也有记录）
  - 书脊有极轻按压态（scale 0.99，支点在底边）；打开某本时其余书淡出并收拢宽度
  - 书架入场有滚动浮现（`fluidReveal`，自左而右错开）
- **点击书 = 书翻开的动画走完后跳到 `zine.html?year=YYYY` 画册**（2026-07-30）。
  书架的翻开动画特意留着，和进入画册连成一个动作；返回时靠 `pageshow` 复位书架。
  ⬇️ 以下是**退役的页内阅读视图**说明（代码保留作回退、已无人调用）：
...
- **抽象拟物**（与书架式 Archive 同一哲学，不画写实质感）：
  ...
  - 触屏两步交互与 Archive 同构：第一击 = **原地抽出片头**（筒钉在原地、片头只往右伸，
    不做任何整排平移——理由同 Archive 的"被掰开"教训），再点才翻开；点其他筒切换预览、点空白收回
  - **分排 = 型号内按宽度装箱**（2026-08 改，与书架同一套思路）：约束 = 任何一卷抽出片头后
    右端仍在屏幕内（展开宽随画幅不同）；红线 = 永远不把不同型号凑进同一排；
    尾排孤卷自动匀平（[3,1]→[2,2]）；型号卷数攒多后手机桌面都自动换排
...
- **点击筒 = 抽出动画走完后跳到 `zine.html?film=卷ID` 画册**（2026-07-30）。
  片框（齿孔 + 片边字）跟着搬进画册了，是 Film 的身份；返回时靠 `pageshow` 复位柜子。
  ⬇️ 以下是**退役的页内阅读视图**说明（代码保留作回退、已无人调用，片框规则仍是有效参考）：
...
### 关于页 `about.html`
- 单列窄宽布局（约 600px 阅读宽度）：中文 bio + 英文 bio，无照片、无标签（作者拍板的极简形态）
- bio 已是真实文案（摄影哲学那几段，作者手填）
- **联系方式 = 一排六个社媒图标**（2026-08）：邮箱、Instagram、小红书、抖音、GitHub、LinkedIn
  - 图标 SVG 内嵌（Simple Icons CC0 / Bootstrap Icons MIT），零外部请求；抖音用 TikTok 音符（同一 logo）
  - 统一单色：常态 `site-muted` 灰、悬停黑、按压淡化——**不用平台品牌色**（图标是界面不是"物"）
  - 增删平台：照抄一个 `<a>` 块改链接和 SVG 即可（文件内有注释）
...
## 画册式阅读（zine.html，全站三个板块已统一）

> 2026-07-30 的大改版：点开任何一个"本子"不再是"网格/页内阅读视图"，
> 而是**一本往下滚的画册**——每屏一张照片（100dvh + scroll-snap 吸附），滑一下翻一页。
> Gallery 先转正，同日 Archive / Film 跟进，三个板块现在共用 `zine.html` 一个模板。
> 旧的阅读方式（series.html 的 Lightbox、archive/film 页内阅读视图）保留作回退、无入口。
...
| 网址 | 是什么 | 入口页 |
|---|---|---|
| `zine.html?id=系列ID` | Gallery 一个系列（分章系列自动插章节扉页） | gallery.html 卡片 |
| `zine.html?year=YYYY` | Archive 一年的散片 | archive.html 点开一本书 |
| `zine.html?film=卷ID` | Film 一卷（照片嵌在片框里） | film.html 点开一个筒 |

**每一本从此有了自己的网址**——这是"每卷/每年可以单独分享"的前提，
也是将来做"每本专属微信卡片"（静态多页方案）的第一步已经就位。

**代码结构**：三个 `xxxBook()` spec 函数各自把本板块的数据翻译成统一的
「封面 + 条目流 + 版权页」，排版引擎只认这套统一格式。**加新板块 = 加一个 spec 函数，
排版代码一行不用动**。三个板块的差异只在 spec 里：Archive 把 note 映射成图注
（它的定位是"只放照片"，不上文字面板）；Film 走专用的 `filmPhotoPage`（片框）。
...
- **Film 片框**（`.film-frame`）：照片嵌在底片上——135 上下齿孔 + 片边字、120 素黑宽边，
  竖片时齿孔转到左右两侧。从 film.html 阅读视图移植（那边是单例 id，这里是类）；
  **横竖判断查 ZINE_DIMS 宽高表**，不用等图片加载完再翻转。片边字走 `font-shell`
  （物体表面印字的全站例外）。齿孔预填 40 颗，靠 `width:0 + min-width:100%` 不参与
  片框宽度计算、多余的靠 overflow 裁掉——**这行别动，去掉会把格子撑满全屏**
- **不引 fluid.js**：翻页交给浏览器原生滚动 + 吸附，这是有意的架构选择（没有代码的地方没有 bug）；
  别为了"统一"把手势引擎接进来
- **对旧体系的改动都很小**：gallery.html 卡片的 href 一行；archive.html 的 `openBook()`
  和 film.html 的 `openRoll()` 各改成"动画走完 → 跳 zine.html"。三处旧阅读代码
  一个字没动、保留作回退（已无人调用）。**书架/胶卷柜的"翻开"动画特意留着**——
  它和进入画册连成一个动作。⚠️ 两个入口页都加了 `pageshow` 复位：从画册按返回
  回来时浏览器会用 bfcache 原样恢复页面，不复位就停在"翻开一半"的中间态
- **fluid.js 仍不能删**：archive/film 的旧阅读视图代码还引着它（虽然不再被调用）；
  真要删得先清理那三处死代码
...
### 之后：上 CDN（作者已拍板，2026-07-26）

背景：从英国实测下载只有 ~200-400KB/s 且不稳（作者人在英国，这个数字混着
跨境链路和服务器带宽两个因素，**国内真实速度待朋友实测**，套餐标称带宽
可在腾讯云控制台查）；无论国内实测如何，CDN 都是流量起来后的正解；
升带宽性价比差（¥660/年买 1.6 倍），CDN 按量计费且境内节点带宽近乎无限。
**先走完上面三步、用一两天确认直连正常，再叠 CDN**——一次只改一层。

腾讯云 CDN 控制台操作要点（¥0.2/GB 左右，他这个流量级 = 每月几毛钱）：
1. 添加域名：业务类型「CDN 网页小文件」、源站 = 自有源 IP `124.221.92.171`、
   回源协议 HTTP、回源 HOST = `yangzhaophoto.com`
2. 缓存配置选**遵循源站**（Nginx 已发 30d/12h 缓存头），只加一条例外：
   路径 `/api/` 缓存 0 秒——留言表单是动态的
3. HTTPS：部署免费证书（**90 天有效，勾自动续期**）+ 强制跳转 + HTTP/2
4. 🔴 **用量封顶必须配**：日流量 5GB 封顶、超了自动关 CDN 回落直连
   ——防"流量盗刷"刷爆账单，这是钱包保险丝，不配不许切 DNS
5. DNSPod：`@` 的 A 记录**暂停（别删，是回滚开关）**，新增 CNAME 指向
   CDN 分配的 `*.cdn.dnsv1.com` 地址
6. 验证：dig 看解析、翻照片、发一条测试留言（确认 /api/ 没被缓存）、微信卡片
7. **部署后改过的 html/js 要去控制台「刷新预热」**（照片不用——只增不改）；
   deploy.sh 自动调刷新 API 需要管理云密钥，部署频率高了再考虑

deploy.sh 和 SSH 都直连 IP，与 CDN 无关，部署流程不变。
...
### 待作者决定
- [ ] **三处退役阅读代码何时删** —— series.html 整个文件、archive.html / film.html 里
      页内阅读视图那几百行。画册用稳一段时间后再清；清完 `fluid.js` 也就没人用了，
      可一并删除（现在还不能删）。删之前它们是"改一处即回旧体验"的回退
...
- [ ] 关于页 `og:description` 更新（bio 本体已填好；og 标签仍是占位的
      "关于我，以及联系方式。"，等作者定一句对外简介后替换）
- [ ] Film 逐卷填 `title` / `desc` / `lens`（都可选，留空自动隐藏）

### 待观察
- [ ] **国内真实速度待验证**（2026-07-27 朋友实测）：作者人在英国，隔着跨境链路
      测不准（实测 RTT 380ms、首字节 0.7s、还遇到传输中途被重置）。朋友用微信
      打开 archive/film 各翻十张，回答三问：首张等几秒 / 顺翻是否秒开 / 有无压暗。
      结果决定两件事：CDN 的紧迫度、GitHub Pages 去留（它对海外的作者本人
      是最快镜像——github.io 在英国畅通，只有留言表单在那边是坏的）
