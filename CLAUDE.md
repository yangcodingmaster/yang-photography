...
│       ├── README.txt
│       ├── profile.jpg             # 关于页个人照片
│       ├── Defocused/              # Gallery 普通系列（id 与 data.js 一致）
│       │   ├── 01.jpeg             # 第一张 = gallery 封面（无需 cover.jpg）
│       │   └── 02–06.jpeg
│       ├── photos-of-2025/         # Gallery 分章系列（id 与文件夹名一致）
...
**命名规则：**
- 系列文件夹名必须和 `data.js` 里的 `id` 字段**完全一致**（大小写、空格）
- 普通系列：`allPhotos` 的**第一张照片**自动作为 gallery 封面，无需单独 `cover.jpg`
- 分章系列（`type: 'sectioned'`）：必须在 `allSeries` 里显式指定 `cover` 字段
...
- 封面图取法：`series.cover`（分章系列）或 `allPhotos[id][0].src`（普通系列）
- 悬停有轻微放大 + 半透明遮罩提示
...
```javascript
{
  ...
  cover:    'assets/images/.../cover.jpg',  // 必须显式指定封面
}
```
...
1. 在 `assets/images/` 下新建文件夹，名字与计划使用的 `id` 完全一致
2. 放入照片，第一张命名为 `01.jpg`（它将自动成为 gallery 封面）
3. 打开 `data.js`，在 `allSeries` 末尾添加新对象（不加 `type` / `cover` 字段）
4. 在 `allPhotos` 里添加对应的 key 和照片数组
5. 刷新 `gallery.html` 查看效果

### 如何添加一个分章系列

1. 在 `assets/images/` 下建系列总文件夹，内部按章节建子文件夹
2. 准备一张封面图（可以是任意一张照片）
3. 在 `allSeries` 里加上 `type: 'sectioned'` 和 `cover: '路径'`
4. 在 `allChapters` 里添加对应的 key 和章节数组
5. 每个章节包含 `id`、`titleZh`、`titleEn`、`descZh`、`descEn`、`photos` 数组

### 如何往 Journal 加照片

1. 把照片放进 `assets/images/Journal/YYYY/`（年份是哪年就放哪年的文件夹）
2. 文件名随意（Journal 不依赖文件名顺序，靠数组顺序决定显示顺序）
3. 在 `data.js` 的 `allJournal['YYYY']` 数组**最前面**加一行 `{ src: '...', alt: '' }`
...
- 不为分章系列省略 `cover` 字段（gallery 封面取不到会报错）
